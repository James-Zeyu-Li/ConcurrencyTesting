...
def generate_plots_for_io(df, csv_filename):
    ...
    sns.scatterplot(data=df, x='TotalThreads', y='TotalTime(us)', hue='LockType', style='LockType', size='OperationCount', sizes=(50, 300), alpha=0.7)
    for i, row in df.iterrows():
        plt.text(row['TotalThreads'], row['TotalTime(us)'], f"C:{row['ConsumerCount']}\nR:{row.get('ReaderCount', '-')}", fontsize=8, ha='right', alpha=0.7)
    ...
    plt.legend(title='LockType / OperationCount', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    ...
    if "MutexContention" in df.columns or "ReadContention" in df.columns or "WriteContention" in df.columns:
        melted_df = df.melt(id_vars=['LockType'], value_vars=['MutexContention', 'ReadContention', 'WriteContention'], var_name='ContentionType', value_name='ContentionCount')
        melted_df['ContentionType'] = melted_df['ContentionType'].replace({'MutexContention': 'Mutex', 'ReadContention': 'Read', 'WriteContention': 'Write'})
        fig2 = plt.figure(figsize=(12, 6))
        sns.barplot(data=melted_df, x='LockType', y='ContentionCount', hue='ContentionType', palette='Set3')
    ...
    sns.lineplot(data=df, x='OperationCount', y='TotalWriteTime(us)', hue='LockType', style='LockType', ci=None, linestyle='--', label='Total Write Time')
    sns.lineplot(data=df, x='OperationCount', y='TotalReadTime(us)', hue='LockType', style='LockType', ci=None, linestyle='-', label='Total Read Time')
    plt.title('OperationCount vs TotalReadTime & TotalWriteTime', fontsize=16)
    plt.xlabel('Operation Count (log scale)', fontsize=14)
    plt.ylabel('Total Time (us)', fontsize=14)
    plt.xscale('log')
    plt.legend(title='Metric / Lock Type', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    ...
    melted_df = df.melt(id_vars=['TotalThreads', 'LockType'], value_vars=['MutexContention', 'ReadContention', 'WriteContention'], var_name='ContentionType', value_name='ContentionCount')
    melted_df['ContentionType'] = melted_df['ContentionType'].replace({'MutexContention': 'Mutex', 'ReadContention': 'Read', 'WriteContention': 'Write'})
    sns.lineplot(data=melted_df, x='TotalThreads', y='ContentionCount', hue='ContentionType', style='LockType', markers=True, dashes=False)
    plt.title('Contention vs TotalThreads', fontsize=16)
    plt.xlabel('Total Threads (Consumer + Reader)', fontsize=14)
    plt.ylabel('Contention Count', fontsize=14)
    plt.legend(title='Contention Type / Lock Type', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    ...
    sns.boxplot(data=df, x='ConsumerCount', y='TotalWriteTime(us)', hue='LockType', palette='Set2')
    ...
    sns.boxplot(data=df, x='ReaderCount', y='TotalReadTime(us)', hue='LockType', palette='Set3')
